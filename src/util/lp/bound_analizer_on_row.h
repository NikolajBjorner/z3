...
class bound_analizer_on_row {
    ...
    X m_bound_minus = zero_of_type<X>();
    ...
    X m_bound_plus = zero_of_type<X>(); // the partial sum for plus, seen so far
...
public :
    ...
    void analyze_bound_on_pivot_row_one_var_case_boxed_fixed(int j, const T & a);
    void analyze_bound_on_pivot_row_one_var_case_low_upper(const T& a,
